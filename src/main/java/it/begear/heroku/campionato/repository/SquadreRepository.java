...
public interface SquadreRepository extends JpaRepository<Squadre, Integer> {
	...
	@Query("SELECT s FROM Squadre s WHERE CONCAT(s.id_squadra, s.nome, s.partite_giocate, s.punti) LIKE %?1% ORDER BY s.punti")
	public List<Squadre> search(String keyword);

	@Query("FROM Squadre")
	public List<Squadre> orderByPunti(Sort sort);
}
